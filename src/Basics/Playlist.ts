...
export class Playlist implements BasicData {
  ...
  updateGenres() {
    ...
    this.songs.forEach((s) => {
      s.getGenres().forEach((g) => {
        if ((this.genres.find((x: Genre) => x.getName() === g.getName())) === undefined) {
          this.genres.push(GenresManager.getGenresManager().getGenreByName(g.getName()) as Genre);
        }
      });
    });
  }
  ...
  public getSongsNames(order: Order = 0): string[] {
    switch (order) {
      ...
      case 2:
        this.songs.sort(function(a, b) {
          if (a.getNameAuthor() < b.getNameAuthor()) {
            return -1;
          }
          if (a.getNameAuthor() > b.getNameAuthor()) {
            return 1;
      ...
      case 10:
        this.songs.sort(function(a, b) {
          if (a.getReproduction() < b.getReproduction()) {
            return -1;
          }
          if (a.getReproduction() > b.getReproduction()) {
            return 1;
